...
public:
    ...
    void fillArrayInOrder(T* array, int size) const;
...
private:
    ...
    void fillArrayInOrderAux(Node *node, T *array, int &index, int size) const;
...
template<class Key, class T>
void AVLTree<Key, T>::fillArrayInOrder(T* array, int size) const {
    int index = 0;
    fillArrayInOrderAux(m_root, array, index, size);
}

template<class Key, class T>
void AVLTree<Key, T>::fillArrayInOrderAux(AVLTree::Node *node, T *array, int &index, int size) const {
    if (node == nullptr || index >= size) {
        return;
    }
    fillArrayInOrderAux(node->m_left, array, index, size);
    if (index < size) {
        array[index++] = node->m_data;
    }
    fillArrayInOrderAux(node->m_right, array, index, size);
}
